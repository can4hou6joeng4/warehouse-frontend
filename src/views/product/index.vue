<template>
  ...
  <div>
    <el-form class="searchForm">
      ...
      <el-form-item style="margin-left: 10px;">
        <el-input v-model="params.productName" placeholder="产品名称" style="width: 120px;"  clearable></el-input>
      </el-form-item>
      <el-form-item style="margin-left: 10px;">
    ...
    <div>
      <el-button type="primary" @click="openProductAdd()">
        <el-icon>
  ...
  <el-table ref="multipleTableRef" :data="commodityPageList" @selection-change="handleSelectionChange" style="width: 100%; margin-top: 10px;" table-layout="auto" size="large" border stripe id="elTable">
    ...
    <el-table-column prop="productNum" label="产品在库数量" sortable />
  ...
  <!-- 添加产品对话框 -->
  <product-add ref="productAddRef" @ok="getProductPageList"></product-add>
  
  <!-- 修改产品对话框 -->
  <product-update ref="productUpdateRef" @ok="getProductPageList"></product-update>
...
const openProductAdd = () => {
  productAddRef.value.open(storeList, unitList);
};
...
const openProductUpdate = (product) => {
  productUpdateRef.value.open(product, storeList, unitList);
};
...
const deleteProduct = (productId) => {
  del(`/product/product-delete/${productId}`, null, { title: "提示", message: "您确定删除该产品吗？" }).then(result => {
